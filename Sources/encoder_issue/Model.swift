...
public protocol Model: Codable {
  ...
  func save<I: Identifier>(using db: Database?, _ onCompletion: @escaping (IdentifiedModel<I, Self>?, RequestError?) -> Void)
  ...
  // Commenting the two lines below it builds
...
public extension Model {
  ...
  func save<I: Identifier>(using db: Database? = nil, _ onCompletion: @escaping (IdentifiedModel<I, Self>?, RequestError?) -> Void) {
    guard let database = db ?? Database.default else {
      onCompletion(nil, .ormDatabaseNotInitialized)
      return
    }
      ...
      if let error = error {
        ...
        connection.execute(query: query) { result in
          ...
          guard let rows = result.asRows, rows.count > 0 else {
            onCompletion(nil, RequestError(.ormNotFound, reason: "Could not retrieve id value for: \(String(describing: self))"))
            return
          }
          ...
          do {
            ...
            let identifiedModel = IdentifiedModel(id: identifier, model: self)
            onCompletion(identifiedModel, nil)
          } catch {
